...
fn format_hex(data: &[u8], label: &str) -> String {
    ...
    // For multi-line output, put hex on the next line
    let mut result = String::new();
    result.push('\n');

    // Subsequent lines with no indent (matching existing format)
...
fn convert_test_vector_to_markdown(tv: &TestVector) -> String {
    ...
    // Ephemeral key material (ikmE is the randomness used)
